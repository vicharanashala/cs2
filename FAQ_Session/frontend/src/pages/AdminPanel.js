...
const AdminPanel = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:5000/faq/${id}`, getAuthHeader());
  ...
  const handleUpdate = async (id) => {
    try {
      await axios.put(`http://localhost:5000/faq/${id}`, editData, getAuthHeader());
  ...
  return (
    <div style={styles.container}>
      ...
      {faqs.length === 0 ? (
        <p style={styles.empty}>No FAQs yet!</p>
      ) : (
        faqs.map((faq, index) => (
            ...
            {editId === faq._id ? (
              ...
              <div>
                ...
                <small style={styles.meta}>
                  By: {faq.createdBy || 'Anonymous'} | Replies: {faq.replies?.length || 0}
                </small>
                <div style={styles.btnGroup}>
...
const styles = {
  ...
  meta: { color: '#888', fontSize: '12px', display: 'block', marginBottom: '10px' },
  ...
  input: { width: '100%', padding: '10px', fontSize: '15px', borderRadius: '8px', border: '1px solid #ddd', marginBottom: '10px', boxSizing: 'border-box' },
  textarea: { width: '100%', padding: '10px', fontSize: '15px', borderRadius: '8px', border: '1px solid #ddd', height: '100px', marginBottom: '10px', resize: 'vertical', boxSizing: 'border-box' }
};
